<!-- M&M guessing form for holiday contests -->
<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<title>Guess the M&M's!</title>
	<style>
		@font-face {
   			font-family: "Readex";
...
<body>
	<?php 
		if(isset($_POST["submit"])){
			...
			$GuessMM = stripslashes($_POST["guess_mm"]);
			// Replace any '~' characters with '-' characters
			$FirstName = str_replace("~", "-", $FirstName);
			$LastName = str_replace("~", "-", $LastName);
			$EmpID = str_replace("~", "-", $EmpID);
			$Department = str_replace("~", "-", $Department);
			$GuessMM = str_replace("~", "-", $GuessMM);

			$ExistingEmpID = array();
			if(file_exists("ContestData/mm.txt") && filesize("ContestData/mm.txt") > 0){
				$MessageArray = file("ContestData/mm.txt");
			...
			if(in_array($EmpID, $ExistingEmpID)){
				...
				$MessageRecord = "$FirstName~$LastName~$EmpID~$Department~$GuessMM\n";
				$MessageFile = fopen("ContestData/mm.txt", "ab");
	 ...
	 <h1>Guess the M&M's!</h1>
	 <p>Win a prize, guess the number of M&M's in the hat!</p>
	 <ul>
	 ...
	 <hr/>
	 <form action="mm_hat.php" method="POST">
	 	...
	 	<input type="text" name="guess_mm" value="<?php echo $GuessMM; ?>" /><br/>
